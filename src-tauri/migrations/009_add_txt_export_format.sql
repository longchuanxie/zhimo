-- 009: 放开 export_tasks.export_format CHECK 约束,支持 txt
-- 背景:006 迁移的 CHECK 限制 export_format IN ('markdown','word','latex','docx')
-- SQLite 不支持 ALTER TABLE DROP CONSTRAINT,通过标准重建表流程实现
-- 注意:tauri-plugin-sql 已自动为每个迁移包裹事务,禁止使用显式 BEGIN/COMMIT

-- 1. 创建新表(放宽 CHECK 为 5 种格式,保留 export_options 列)
CREATE TABLE export_tasks_new (
  id TEXT PRIMARY KEY,
  project_id TEXT NOT NULL,
  export_scope TEXT NOT NULL,
  export_format TEXT NOT NULL CHECK (export_format IN ('markdown','txt','word','latex','docx')),
  document_ids TEXT,
